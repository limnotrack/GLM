...
!###############################################################################
SUBROUTINE aed_init_glm(i_fname, len, NumWQ_Vars, NumWQ_Ben, write_mode)       &
                                                      BIND(C, name=_WQ_INIT_GLM)
!-------------------------------------------------------------------------------
! Initialize the GLM-AED driver by reading settings from aed.nml.
! If write_mode /= 0 (--write_nml), a baseline aed.nml is written instead,
! covering a fixed curated set of modules rather than reading an existing one.
!-------------------------------------------------------------------------------
!ARGUMENTS
   CCHARACTER,INTENT(in) :: i_fname(*)
   CSIZET,INTENT(in)     :: len
   CINTEGER,INTENT(out)  :: NumWQ_Vars, NumWQ_Ben
   CINTEGER,INTENT(in)   :: write_mode
!
!LOCALS
...
!BEGIN
   ...
   aed_write_nml_mode = (write_mode /= 0)
...
# endif
   ...
   IF ( aed_write_nml_mode ) THEN
      print *,"     Writing baseline aed_models config to ",TRIM(fname)
      OPEN(namlst,file=fname,action='write',status='replace',iostat=status)
      IF ( status /= 0 ) CALL STOPIT("Cannot open file " // TRIM(fname) // " for writing")

      !# Curated baseline module set (see GLM's --write_nml / --help)
      models(1) = 'aed_sedflux'
      models(2) = 'aed_carbon'
      models(3) = 'aed_oxygen'
      models(4) = 'aed_silica'
      models(5) = 'aed_nitrogen'
      models(6) = 'aed_phosphorus'
      models(7) = 'aed_organic_matter'
      models(8) = 'aed_phytoplankton'
      models(9) = 'aed_totals'
      WRITE(namlst,'(A)') "! Baseline AED configuration, generated by `glm --write_nml`."
      WRITE(namlst,'(A)') "! Covers this baseline's default module set only - add/remove names from"
      WRITE(namlst,'(A)') "! the 'models' list below (and their matching sections) for other modules."
      WRITE(namlst,'(A)') "&aed_models"
      CALL aed_nml_wsa(namlst, 'models', models, 9, 'Active AED modules, in the order their sections appear below')
      WRITE(namlst,'(A)') "/"
      WRITE(namlst,'(A)') ""
   ELSE
